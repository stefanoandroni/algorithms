...
public class SymmetricMatrix {
    ...
    public static <T extends Comparable<T>> boolean isSymmetric(T M [][]) {
        int r = 0;
        int c = 1; // it is not useful to check the main diagonal M[r][c] == M[c][r] if r == c
        int n = M.length;
        ...
        while (r < n - 1 && M[r][c].compareTo(M[c][r]) == 0) {
            ...
            if (c >= n) {
                r ++;
                c = r + 1; // it is not useful to check if M[c][r]==M[r][c] if it has already been checked M[r][c]==M[c][r]
            }
        }
